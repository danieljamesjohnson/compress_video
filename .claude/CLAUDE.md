...
- **Mac workflow (03-01 task 1, 2026-09-25): `bash tool/mac_sync.sh` then `bash tool/mac_run.sh <build-ios|build-macos|ios <suite>|macos|xctest-ios|xctest-macos|shell ...>`.** Sync mirrors the working tree (not git) to `dans-macbook-air:~/CodeProjects/compress-video`; run executes one alarm-bounded command there (`MAC_RUN_TIMEOUT`, default 540 s, exit 142 on alarm, whole remote process group killed) with `~/development/flutter-stable/bin` (3.47.5 stable, the only SDK to use -- never Dan's `~/flutter`) and `/opt/homebrew/bin` on PATH. **Homebrew 5.1.15 and CocoaPods 1.17.0 ARE now installed on the Mac** (QUESTIONS.md #7 resolved) -- plain `ssh ... which pod` says otherwise only because non-interactive SSH lacks `/opt/homebrew/bin`. The Mac still sleeps on battery within ~30 min of a wake (it went down mid-`flutter build macos` on 2026-09-25); probe first, never loop. Each SSH round trip costs ~11-13 s on this tailnet. Lesson from the first `build-ios` there: the Mac's Xcode hits Swift's "unable to type-check this expression in reasonable time" limit where CI's faster runner does not -- keep `??`/operator chains short (`fac7f69`).
- **A real, measured Apple-vs-Android bitrate divergence exists and is intentionally NOT gated to equality (03-07/03-08).** Apple overshoots the nominal preset bitrate target by 3-10% (both iOS Simulator and macOS host, `doc/PRESETS.md`); Android's software emulator encoder undershoots by as much as 71%. `outputBytes` and `elapsedMs` are excluded from the compression parity gate's exact/tolerance comparisons entirely (only a documented ±50% envelope on bytes catches an order-of-magnitude regression) — this is the same "different by design" wording now in the README's "What 'the same on every platform' means" section and `CompressResult`'s dartdoc. Do not tighten this gate to chase byte parity; the two engines are not supposed to agree there.
- **BULD-04's Apple half (the example app driven by hand on the iOS simulator and macOS host, with screenshots) needs the Mac and could not be done in 03-09** — the Mac was unreachable the whole session. CI's `integration_test` suites on the simulator/macOS host prove the PLUGIN engine on Apple; they call `CompressVideo.compress()` etc. directly and never import or drive `example/lib/src/main_screen.dart`, so they are not evidence for BULD-04's own claim about the example app's UI. Whoever resumes this: `bash tool/mac_sync.sh && bash tool/mac_run.sh ios integration_test/compress_test.dart` is 03-09 task 1's own precondition — run that first, then task 1 exactly as `03-09-PLAN.md` specifies (it was never started; no code or commit exists for it).
