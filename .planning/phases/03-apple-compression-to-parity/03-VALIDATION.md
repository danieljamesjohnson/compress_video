...
| 3-05-03 | 05 | 4 | CORE-01 | T-03-23 | Passthrough is chosen only after checking the source track's media subtype; anything else falls back to an AAC re-encode and reports it, so no unplayable track can ship | integration | `bash tool/mac_sync.sh && bash tool/mac_run.sh ios integration_test/compress_audio_test.dart && bash tool/mac_run.sh macos integration_test/compress_audio_test.dart && cd example && flutter test integration_test/compress_audio_test.dart -d emulator-5554` | ✅ | ⚠️ HALTED — CI run 35821995638: passthrough/strip/no-audio-track/1000bps-clamp all pass (4/7); three forced-reencode cases still fail (bitrate measured 24,182bps regardless of 64,000/128,000bps request; 1-channel request read as 2 channels). Fix pushed (commit 8a44a04: AVEncoderBitRateStrategyKey constant + esds-authoritative channel read) but UNVERIFIED — every run since (35826458554 attempts 1-2) was consumed by hosted-simulator launch hangs before compress_audio_test.dart could run. Next step: `bash tool/mac_run.sh ios integration_test/compress_audio_test.dart` on a real Mac once reachable (03-01 task 1, QUESTIONS.md #7/#8) |
