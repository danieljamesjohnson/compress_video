...
- The plan's own acceptance criterion `grep -c 'nyquist_compliant: true' 01-VALIDATION.md` is `1` was unsatisfiable as originally written: the file's Per-Task Verification Map row `1-07-03` and its own Sign-Off checklist BOTH already quote that exact grep command as documentation text (pre-dating this plan — present in the file before any 01-07 edits), so the real count is `3` (frontmatter + the two self-referential mentions), not `1`. Same class of off-by-count authoring quirk 01-01-SUMMARY.md and 02-01-SUMMARY.md already documented for their own plans. The frontmatter value itself (`nyquist_compliant: true`, exactly once) is what actually matters and is correctly set.
